(function() {
  ...
  function togglePlayPauseButton() {
    ...
    if (elem.className.includes(' is-play')) {
  ...
  function openActiveSections() {
    var arrGroupContainers = document.getElementsByClassName(
  ...
  function setActiveBeatIndex() {
    activeBeatIndex = document
      .querySelector('input[name="beatselector"]:checked')
  ...
  arrToggleBtns.forEach(function(v, i, a) {
    //add the handlers used for toggling elements by adding an 'act' class
    v.addEventListener('click', function() {
    ...
    v.addEventListener('click', function() {
      ...
      if (
        document
          ...
          .firstElementChild.className.indexOf('is-play') < 0
      ) {
        stopTheBiets();
